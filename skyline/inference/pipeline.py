...
from ouluskyline.data.utils import read_dicom, process_xray

from ouluskyline.inference.utils import fuse_bn_recursively
from ouluskyline.train.augmentation import CenterCrop
from ouluskyline.train.dataset import get_pair
...
class KneeNetEnsemble(nn.Module):
    def __init__(self, snapshots_paths, mean_std_path, device=None):
        super().__init__()
        self.states = []
        self.logger = logging.getLogger(f'skyline-backend:pipeline')
        if device is None:
...
def parse_args():
    ...
    parser.add_argument('--output_dir', default='../../../skyline_test_output', help='Stores heatmaps')
    parser.add_argument('--output_csv', default='../../../skyline_test_output/preds.csv', help='Stores predictions')
